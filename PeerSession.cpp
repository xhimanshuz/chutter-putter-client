...
PeerSession::PeerSession(std::shared_ptr<boost::asio::io_context> _ioc, std::array< std::string, 5> askPeerData, unsigned short localPort, QObject *parent): ioc{_ioc},
    ...
    public_ip{askPeerData[1]}, private_port{std::atoi(askPeerData[2].c_str())}, public_port{std::atoi(askPeerData[3].c_str())}, username{askPeerData[4].c_str()}, QObject(parent)
{
...
//            socket->close();
    ...
    std::cout << "[~] doConnect: "<< IS_CONNECTED<<std::endl;;
}
